...
const STORAGE_KEY = "intra_therapists_v1";
      ...
      <aside className={`drawer ${t ? "open" : ""}`}>
        ...
        <div className="drawer-body">
          ...
          <div className="cta-row">
            <button className="btn-primary" onClick={() => window.__toast(`Solicitud de cita enviada para ${t.nombre.split(" ")[0]}`)}>
...
const EMPTY = { nombre: "", titulo: "Licenciatura", cedula: "", preparacion: "", formacion: "" };
...
function TherapistForm({ open, initial, onClose, onSave }) {
  ...
  // Preview avatar / matched areas for the LIVE form
...
function App() {
  const [therapists, setTherapists] = useState(() => {
    ...
    return window.THERAPISTS;
  });

  useEffect(() => {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(therapists)); } catch(e) {}
  }, [therapists]);
  ...
  const saveTherapist = (data) => {
    if (editingT) {
      setTherapists(prev => prev.map(t => t.nombre === editingT.nombre ? data : t));
      window.__toast(`${data.nombre.split(" ")[0]} actualizado`);
    } else {
      if (therapists.some(t => t.nombre.toLowerCase() === data.nombre.toLowerCase())) {
        alert("Ya existe un terapeuta con ese nombre.");
        return;
      }
      setTherapists(prev => [data, ...prev]);
  ...
  const deleteTherapist = (t) => {
    setTherapists(prev => prev.filter(x => x.nombre !== t.nombre));
    window.__toast(`${t.nombre.split(" ")[0]} eliminado`);
  };

  const resetData = () => {
    if (confirm("¿Restaurar el catálogo original? Se perderán los cambios locales.")) {
      setTherapists(window.THERAPISTS);
      window.__toast("Catálogo restaurado");
    }
  };
  ...
  return (
    <div className="shell">
        ...
        <div className="hero-row">
          ...
          <div style={{ flex: 1, minWidth: 280 }}>
            ...
            <div className="hero-actions">
              ...
              <button className="hero-btn ghost" onClick={resetData} title="Restaurar al catálogo original">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round"><path d="M3 12a9 9 0 1 0 3-6.7L3 8"/><path d="M3 3v5h5"/></svg>
                Restaurar
              </button>
      ...
      {/* Floating "Agregar" FAB for quick access */}
